...
function animateValue({ start = 0, end = 100, duration = 1000, delay = 0, ease = easeOutCubic, onUpdate, onEnd }: AnimateValueProps) {
  ...
  function tick() {
    ...
    onUpdate((start + (end - start) * ease(t)).toFixed(3));
...
export default function BorderGlow({
  ...
  useEffect(() => {
    ...
    animateValue({ ease: easeInCubic, duration: 1500, end: 50, onUpdate: v => {
      card.style.setProperty('--cursor-angle', `${((angleEnd - angleStart) * (parseFloat(v) / 100) + angleStart).toFixed(3)}deg`);
    }});
  ...
  return (
    <div
      ...
      style={{
        ...
        '--fill-display': fillOpacity === 0 ? 'none' : 'block',
        ...glowVars,
